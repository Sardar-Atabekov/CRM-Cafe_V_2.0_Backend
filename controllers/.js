...
const AdminScheme = require('../models/user');
      ...
      if (err) {
        ...
        if (data.length !== 0) {
          ...
          const mapdata = data.map((item, idx) => {
            ...
            idies.push(String(item.createdBy));
            return item;
          });
          AdminScheme.find({
            '_id': {
            ...
            if(err) {
              ...
              AdminScheme.find({
                ...
                if(err) res.json({success: false, message: "Что-то пошло не так"});
                else {
                  res.json({
                    users: data,
                    docs: mapdata
...
exports.getOneFile = async(req, res) => {
  ...
  FileScheme.findOne({
    ...
    doc.save();
    res.sendFile(`./uploads/${createdBy}/${src}`);
...
exports.addToArchive = async (req,res) => {
  ...
  if(userId) {
    if(filesId.length > 0) {
      try {
        FileScheme.updateMany(
          {
            _id:{$in: filesId},
            createdBy: userId,
          }
          ,{archive: true},(err,data) => {
          if(err) {
            res.json({
              success: false,
              message: "Что-то пошло не так"
